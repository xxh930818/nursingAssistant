          ...
          <div className="flex w-full items-center gap-4">
            ...
            <div className="flex flex-col flex-1">
              <div className="flex items-center gap-2">
                <p className="text-slate-900 text-xl font-bold">{profile.name}</p>
                <span className="bg-blue-600/10 text-blue-600 text-[10px] px-2 py-0.5 rounded-full font-bold">
                  已实名
                </span>
              </div>
              <p className="text-slate-500 text-sm">{profile.title} | 从业{profile.experience}年 | {profile.specialty}</p>
            </div>
            <button
        ...
        <div className="flex gap-3 p-4 pt-0">
          <div className="flex flex-1 flex-col gap-1 rounded-xl p-4 bg-blue-600/5 border border-blue-600/10">
            ...
            <div className="flex items-end gap-1">
              <p className="text-slate-900 text-2xl font-bold leading-none">{profile.rating}</p>
              <Star className="text-blue-600 w-4 h-4 fill-current pb-0.5" />
            </div>
          </div>
          <div className="flex flex-1 flex-col gap-1 rounded-xl p-4 bg-blue-600/5 border border-blue-600/10">
            <p className="text-slate-500 text-xs font-medium">累计订单</p>
            <p className="text-slate-900 text-2xl font-bold leading-none">{profile.totalOrders}</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl p-4 border border-slate-100">
            ...
            <div className="flex items-end justify-between h-32 gap-2 mb-2 px-2">
              {profile.weeklyHours.map((hours, index) => {
                ...
                const maxHours = Math.max(...profile.weeklyHours);
                const heightPercent = (hours / maxHours) * 100;
                const isToday = index === 4; // 假设今天是周五

                return (
                  <div key={index} className="flex flex-col items-center flex-1 gap-2">
                    <div
                      className={`w-full rounded-t-sm transition-colors cursor-pointer ${isToday ? 'bg-blue-600' : 'bg-blue-600/20'}`}
                      style={{ height: `${heightPercent}%` }}
                      title={`${hours}h`}
                      onClick={() => toast.info(`周${days[index]}: ${hours}小时`)}
                    ></div>
                    <span className={`text-[10px] ${isToday ? 'text-slate-900 font-bold' : 'text-slate-400'}`}>
                      {days[index]}
                    </span>
                  </div>
                );
            ...
            <div className="flex justify-around border-t border-slate-200 pt-4 mt-2">
              <div className="text-center">
                ...
                <p className="text-slate-900 font-bold">{profile.weeklyHours.reduce((a, b) => a + b, 0)}h</p>
              </div>
              <div className="text-center">
        ...
        <div className="px-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div>
                <p className="text-slate-900 text-lg font-bold leading-none">{profile.goodRate}%</p>
            ...
            >
              ...
              <div>
                <p className="text-slate-900 text-lg font-bold leading-none">{profile.badReviews}</p>
        ...
        <div className="px-4 py-2">
          ...
          <div className="space-y-3">
            {profile.certificates.map((cert, index) => (
              <div
                key={index}
                onClick={() => handleAction(`${cert.name}详情`)}
              ...
              >
                <div className="size-12 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400">
                  {index === 0 ? <BadgeCheck className="w-6 h-6" /> : <Stethoscope className="w-6 h-6" />}
                </div>
                <div className="flex-1">
                  <p className="text-slate-900 text-sm font-bold">{cert.name}</p>
                  <p className={`${cert.verified ? 'text-green-500' : 'text-slate-500'} text-xs`}>
                    {cert.verified ? `已核验 · 有效期至 ${cert.validUntil}` : '待核验'}
                  </p>
                </div>
